...
<!-- for negra coref annotations by schiehlen (2004) [xml representation by CC] 
     todo: evaluate annotated semantic types -->
<xsl:template match="/">
    ...
    <rdf:RDF
        ...
        <xsl:call-template name="write-corpus">
            <xsl:with-param name="corpusid">negra_2.0</xsl:with-param>
        </xsl:call-template>
        
        <xsl:call-template name="write-layer">
            <xsl:with-param name="id">syntax</xsl:with-param> <!-- coref directly annotated to NEGRA contituents -->
            <xsl:with-param name="type">StructLayer</xsl:with-param>
            <xsl:with-param name="documentid">negra_2.0</xsl:with-param>
        </xsl:call-template>

    <xsl:for-each select="//*[matches(@id,'^s[0-9][0-9]*')]">
        <xsl:variable name="id" select="concat(@id,'_',@markable)"/>
        <!--xsl:message> <xsl:value-of select="$id"/></xsl:message-->
        <xsl:for-each select="./*">
            <xsl:if test="string-length(@first-mention)=0">
                <!--xsl:call-template name="write-node">
                    <xsl:with-param name="id" select="$id"/>
                    <xsl:with-param name="layerid">coref</xsl:with-param>
                </xsl:call-template> 
                only if relations are on a different layer than the nodes which is not possible at the moment -->
                ...
                <xsl:choose>
                    <xsl:when test="name()='anaphoric'">
                        <!--xsl:message>
                            <xsl:value-of select="concat('anaphoric ',$id)"/>
                        </xsl:message-->
                    <xsl:variable name="refid" select="@refid"/>
                    <xsl:variable name="target">
                        <xsl:for-each select="./preceding::anaphoric[@refid=$refid][1]/..">
                            <xsl:value-of select="concat(@id,'_',@markable)"/>
                        </xsl:for-each>
                    </xsl:variable>
                    <xsl:variable name="relid" select="concat('anaph-',$id,'-to-',$target)"/>
                    <xsl:call-template name="write-point-rel">
                        <xsl:with-param name="id" select="$relid"/>
                        <xsl:with-param name="src" select="$id"/>
                        <xsl:with-param name="tgt" select="$target"/>
                    </xsl:call-template>
                    <xsl:call-template name="assign-annotation">
                        <xsl:with-param name="name">type</xsl:with-param>
                        <xsl:with-param name="attachTo" select="$relid"/>
                        <xsl:with-param name="value">anaphor_antecedent</xsl:with-param>
                    </xsl:call-template>
                </xsl:when>
                <xsl:when test="name()='bridging'">
                    <xsl:variable name="trigger" select="@trigger"/>
                    <xsl:variable name="target">
                        <xsl:for-each select="./preceding::*[(name()='anaphoric' and @refid=$trigger) or (name()='bridging' and @trigger=$trigger)][1]">
                            <xsl:value-of select="concat(../@id,'_',../@markable)"/>
                        </xsl:for-each>
                    </xsl:variable>
                    <xsl:if test="not($target='')">
                        ...
                        <xsl:call-template name="assign-annotation">
                            <xsl:with-param name="name">type</xsl:with-param>
                            <xsl:with-param name="attachTo" select="$relid"/>
                            <xsl:with-param name="value">bridging_trigger</xsl:with-param>
                        </xsl:call-template>
                    </xsl:if>
                </xsl:when>
                <xsl:otherwise>
    ...
    <!-- warning: this should normally not be called, use write-nonterminal/write-terminal instead,
         this is only necessary if an already specified node is assigned to multiple layers -->
    <xsl:template name="write-node">
        <xsl:param name="id"/>
        <xsl:param name="rootid">empty</xsl:param>
        <xsl:param name="layerid">empty</xsl:param>
        <powla:Node rdf:about="{concat('',$id)}">
            <xsl:if test="not($rootid='empty')"><powla:hasLayer rdf:resource="{$layerid}"/></xsl:if>
            <xsl:if test="not($layerid='empty')"><powla:hasRoot rdf:resource="{$rootid}"/></xsl:if>
        </powla:Node>
    </xsl:template>
